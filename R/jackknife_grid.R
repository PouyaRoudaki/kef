...
#'   cloud_computing = FALSE
#' )
jackknife_normalised_weight_error_grid_inner_parallelized <- function(centered_kernel_mat_at_sampled,
                                                           sampled_x,
                                                           min_x,
                                                           max_x,
                                                           lambda_hat_grid,
  ...
  for (outer_index in seq_len(nrow(grid))) {
    ...
    # Export necessary variables to the cluster
    parallel::clusterExport(cl_inner,
                            varlist = c("get_weights_wo_grid","weights_hat_norm",
                                        "centered_kernel_mat_at_sampled",
                                        "sampled_x", "min_x","max_x",
                                        "lambda_hat", "tau_hat"),
                            envir = environment())
    ...
    jackknife_err_vec <- unlist(parallel::parLapply(cl_inner, seq_len(nrow(centered_kernel_mat_at_sampled)), function(i) {
      ...
      w_hat_jackknife_norm <- w_hat_jackknife/(pracma::Norm(w_hat_jackknife))

      one_out_err <- t(w_hat_jackknife_norm - weights_hat_norm[-i]) %*%
        temp_centered_kernel_mat_at_sampled %*%
