...
marginal_log_likelihood <- function(centered_kernel_mat_at_sampled,
  ...
  likelihood_vector_trimmed <- likelihood_vector[(4):(length(likelihood_vector)-3)]
  sampled_x_trimmed <- sampled_x[(4):(length(sampled_x)-3)]
  ...
  while (t <= max.iterations) {
    ...
    p_ml <- plot_ly() %>%
      add_surface(
        x = interp_data$x,
        y = interp_data$y,
        z = matrix(interp_df$z, nrow = length(interp_data$x), ncol = length(interp_data$y)),
        colorscale = 'Viridis'
      ) %>%
      layout(
        title = paste0("Marginal Log Likelihood-Iteration:", t),
        scene = list(
          xaxis = list(title = "Lambda log(λ)"),
          yaxis = list(title = "Tau log(τ)"),
          zaxis = list(title = "Marginal Log Likelihood (MLL)")
        )
      )
