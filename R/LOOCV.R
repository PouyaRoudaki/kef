...
#'   cloud_computing = FALSE
#' )
loocv_error_kme_true_grid_inner_parallelized <- function(centered_kernel_mat_at_sampled,
                                                sampled_x,
                                                min_x,
                                                max_x,
                                                lambda_hat_grid,
  ...
  for (outer_index in seq_len(nrow(grid))) {
    ...
    loocv_err_vec <- unlist(parallel::parLapply(cl_inner, seq_len(nrow(centered_kernel_mat_at_sampled)), function(i) {
      tryCatch({
        ...
        one_out_err <- centered_kernel_mat_at_sampled[i, i] -
          2 * prob_wo_i %*% centered_kernel_mat_at_sampled[-i, i] +
          prob_wo_i %*% centered_kernel_mat_at_sampled[-i, -i] %*% prob_wo_i
