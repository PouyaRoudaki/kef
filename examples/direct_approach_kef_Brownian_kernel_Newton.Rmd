...
### Incorporate the constraint $\sum_{j=1}^m \theta(z_j) = 0$

```{r}
# Initialize the new matrix as a copy of the original matrix
centered_kernel_mat_at_grid_n_1 <- matrix(0, nrow(centered_kernel_mat_at_grid), ncol(centered_kernel_mat_at_grid))

# Get the number of rows and columns
n <- nrow(centered_kernel_mat_at_grid)
m <- ncol(centered_kernel_mat_at_grid)

# Calculate row sums and the sum of the last row
row_sums <- rowSums(centered_kernel_mat_at_grid)
last_row_sum <- sum(centered_kernel_mat_at_grid[n,])

# Vectorized calculation for each entry in the matrix
for (i in 1:n) {
  centered_kernel_mat_at_grid_n_1[i, ] <- centered_kernel_mat_at_grid[i, ] - 
    (row_sums[i] / last_row_sum) * centered_kernel_mat_at_grid[n, ]
}

centered_kernel_mat_at_grid_n_1 <- centered_kernel_mat_at_grid_n_1[-n,]
...
```{r}
w_vec <- rep(0, nrow(centered_kernel_mat_at_grid)-1)
```
...
compute_J_parallel <- function(w_vec, centered_kernel_mat_at_grid) {
    ...
    # Export variables to the cluster, except exp_vec
    clusterExport(cl, c("centered_kernel_mat_at_grid", "w_vec", "n"))
    
    # Use pbSapply to parallelize with a progress bar
    J <- pbsapply(1:n, function(s) {
        # Compute exp_vec within the cluster
        exp_vec <- exp(t(centered_kernel_mat_at_grid) %*% w_vec)
...
J_result <- compute_J_parallel(w_vec = w_vec,centered_kernel_mat_at_grid_n_1)
```
...
exponents_vec <- rep(0, ncol(centered_kernel_mat_at_grid_n_1))

for (j in 1:ncol(centered_kernel_mat_at_grid_n_1)) {
  for (k in 1: nrow(centered_kernel_mat_at_grid_n_1)) {
    exponents_vec[j] <- exponents_vec[j] + centered_kernel_mat_at_grid_n_1[k,j] * w_vec[k]
  }
}

for (j in 1:ncol(centered_kernel_mat_at_grid_n_1)) {
  a <- a + centered_kernel_mat_at_grid_n_1[1,j] * 
    (centered_kernel_mat_at_grid_n_1[1,j] * exp(exponents_vec[j])/sum(exp(exponents_vec)) - exp(exponents_vec[j]) *(sum(exp(exponents_vec) * centered_kernel_mat_at_grid_n_1[1,j]))/sum(exp(exponents_vec))^2)
}

print(a)

J_manual <- matrix(0,nrow(centered_kernel_mat_at_grid_n_1),nrow(centered_kernel_mat_at_grid_n_1))

for (s in 1:nrow(centered_kernel_mat_at_grid_n_1)) {
  for (t in 1:nrow(centered_kernel_mat_at_grid_n_1)) {
    for (j in 1:ncol(centered_kernel_mat_at_grid_n_1)) {
  J_manual[s,t] <- J_manual[s,t] + centered_kernel_mat_at_grid_n_1[s,j] * 
    (centered_kernel_mat_at_grid_n_1[t,j] * exp(exponents_vec[j])/sum(exp(exponents_vec)) - exp(exponents_vec[j]) *(sum(exp(exponents_vec) * centered_kernel_mat_at_grid_n_1[t,j]))/sum(exp(exponents_vec))^2)
    }
  }
  print(paste("row:",s))
}
...
compute_f_parallel <- function(w_vec, centered_kernel_mat_at_grid, kme) {
  ...
  # Export necessary variables to the cluster, excluding exp_vec and exp_sum
  clusterExport(cl, c("centered_kernel_mat_at_grid", "w_vec", "KME_true", "n"))
  
  # Define the function to compute each element of f
  compute_f_element <- function(s) {
    # Calculate exp_vec and exp_sum within the cluster
    exp_vec <- exp(t(centered_kernel_mat_at_grid) %*% w_vec)
    exp_sum <- sum(exp_vec)
...
f_result <- compute_f_parallel(w_vec, centered_kernel_mat_at_grid_n_1, KME_true[-n])
...
library(MASS)
max_iter <- 20
w_vec <- rep(1, length(vec_fixed_points)-1)

for (counter in 1:max_iter) {
  w_vec_old <- w_vec
  
  print("J is under calculation:")
  
  J_result <- compute_J_parallel(w_vec, centered_kernel_mat_at_grid_n_1)
  
  print("J summary:")
  print(summary(as.vector(J_result)))
  
  print("f is under calculation:")
  
  f_result <- compute_f_parallel(w_vec,centered_kernel_mat_at_grid_n_1,KME_true[-n])
  
  print("f summary:")
  print(summary(f_result))
  
  w_vec <- w_vec - 0.001 * ginv(J_result) %*% f_result
