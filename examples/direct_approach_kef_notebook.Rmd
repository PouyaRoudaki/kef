...
vec_fixed_points= rnorm_mixture(n, means, sds, probabilities)
...
KME_true <- sapply(grid_points, function(point) {
  sum <- 0
...
true_KME <- data.frame(grid_points, KME_true)
```
...
results <- sapply(grid_points, function(point) {
...
df_std <- data.frame(grid_points, standard = results)
...
df_combined <- rbind(
  data.frame(grid_points = df_std$grid_points, value = df_std$standard, line = "KME Estimator"),
  data.frame(grid_points = true_KME$grid_points, value = true_KME$KME_true, line = "True KME")
)
...
df_reg <- data.frame(grid_points, regularized = results)
```
...
# Data frame with your data
df_shr <- data.frame(grid_points, shrinkage = results)
...
### Function evaluation on the grid
results <- lapply(grid_points, function(point) {
...
# Data frame with your data
df_bayes <- data.frame(grid_points, post_mean, upper_conf = post_mean+sqrt(post_var), lower_conf = post_mean - sqrt(post_var))
```

## Merge all the KME estimators and plot

```{r}
df_all <- merge(df_std,df_reg,by = "grid_points")
df_all <- merge(df_all,df_shr,by = "grid_points")
df_all <- merge(df_all,df_bayes,by = "grid_points")
df_all <- merge(df_all,true_KME,by = "grid_points")
...
# Plot the results using ggplot
library(ggplot2)
ggplot(df_long, aes(x = grid_points, y = estimation, color = type)) +
  geom_hline(yintercept = 0, color = "gray", linetype = "solid", linewidth = 0.5) +  # Add axis y = 0
  geom_vline(xintercept = 0, color = "gray", linetype = "solid", linewidth = 0.5) +  # Add axis at x = 0
  geom_point(data = df_fixed_points, aes(x = x, y = y), color = 'red', size = 2 ,shape =3) +  # Plot x points with y = 0 in red
...
# Plot the results using ggplot
ggplot(df_long %>% filter(type %in% c("standard","KME_true")), aes(x = grid_points, y = estimation, color = type)) +
  geom_hline(yintercept = 0, color = "gray", linetype = "solid", linewidth = 0.5) +  # Add axis y = 0
  geom_vline(xintercept = 0, color = "gray", linetype = "solid", linewidth = 0.5) +  # Add axis at x = 0
  geom_point(data = df_fixed_points, aes(x = x, y = y), color = 'red', size = 2 ,shape =3) +  # Plot x points with y = 0 in red
  geom_line(linewidth = 1.2) + # Plot kernel mean embedding estimator
...
```{r}
# Plot the results using ggplot
ggplot(df_long %>% filter(type %in% c("regularized","KME_true")), aes(x = grid_points, y = estimation, color = type)) +
  geom_hline(yintercept = 0, color = "gray", linetype = "solid", linewidth = 0.5) +  # Add axis y = 0
  geom_vline(xintercept = 0, color = "gray", linetype = "solid", linewidth = 0.5) +  # Add axis at x = 0
  geom_point(data = df_fixed_points, aes(x = x, y = y), color = 'red', size = 2 ,shape =3) +  # Plot x points with y = 0 in red
  geom_line(linewidth = 1.2) + # Plot kernel mean embedding estimator
...
```{r}
# Plot the results using ggplot
ggplot(df_long %>% filter(type %in% c("shrinkage","KME_true")), aes(x = grid_points, y = estimation, color = type)) +
  geom_hline(yintercept = 0, color = "gray", linetype = "solid", linewidth = 0.5) +  # Add axis y = 0
  geom_vline(xintercept = 0, color = "gray", linetype = "solid", linewidth = 0.5) +  # Add axis at x = 0
  geom_point(data = df_fixed_points, aes(x = x, y = y), color = 'red', size = 2 ,shape =3) +  # Plot x points with y = 0 in red
  geom_line(linewidth = 1.2) + # Plot kernel mean embedding estimator
...
```{r}
# Plot the results using ggplot
ggplot(df_long %>% filter(type %in% c("post_mean",'upper_conf','lower_conf',"KME_true")), aes(x = grid_points, y = estimation, color = type)) +
  geom_hline(yintercept = 0, color = "gray", linetype = "solid", linewidth = 0.5) +  # Add axis y = 0
  geom_vline(xintercept = 0, color = "gray", linetype = "solid", linewidth = 0.5) +  # Add axis at x = 0
  geom_point(data = df_fixed_points, aes(x = x, y = y), color = 'red', size = 2 ,shape =3) +  # Plot x points with y = 0 in red
  geom_line(linewidth = 1.2) + # Plot kernel mean embedding estimator
...
```{r}
p <- solve(centered_kernel_mat_at_sampled) %*% df_shr$shrinkage 
```
...
u_vec <-  one_m %*% centered_kernel_mat_at_grid %*% solve(centered_kernel_mat_at_sampled)
```
...
# Create the ggplot
ggplot(df_result, aes(x = sampled_x, y = estimated_p)) +
  geom_line() +       # Line plot
  geom_point() +      # Add points
  labs(title = "Estimated P versus Sampled X",
       x = "Sampled X", y = "Estimated P") +
  theme_minimal()     # Optional: Minimal theme
```
