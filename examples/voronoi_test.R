...
voronoi_result <- generate_voronoi(points, extension, x_min, x_max, y_min, y_max)

voronoi_result$vertices
voronoi_result$polygons
voronoi_result$site_points
sum(voronoi_result$polygon_areas)
voronoi_result$polygons_on_boundary
...
ggplot() +
  ...
  scale_x_continuous(breaks = seq(-10, 10, by = 2)) +   # Smaller 'by' means denser vertical grid lines
  scale_y_continuous(breaks = seq(-10, 10, by = 2))    # Smaller 'by' means denser horizontal grid lines
