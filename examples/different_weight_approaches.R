...
#optimal_kef <- optimize_marginal_log_likelihood_nloptr(centered_kernel_mat_at_sampled,
#                                 min_x = -3.1,
#                                 max_x = 3.1,
#                                 sampled_x,
#                                 initial_lambda = 1,
...
kef_res <- kef(sampled_x,grid = x_grid,lambda = 0.1, tau = 7.0742076958161e-05)
...
ggplot() +
  ...
  ggtitle(paste('Kernel Density Estimate for lambda_hat =',
                format(1,digits = 3,scientific = T),'and tau_hat =',format(1/1350 * sqrt(1000/100),digits = 3,scientific = T))) +
  xlab('Value') +
  ylab('Density') +
  theme_bw() +
...
ggplot() +
  ...
  scale_color_manual(name = "Type of density", values = c('True Density' = 'red', 'KDE Adaptive' = 'blue','KDE Fixed' = 'limegreen', 'KEF' = 'orange')) +
  ggtitle(paste('Kernel Density Estimate for lambda_hat =',
                format(optimal_kef$lambda,digits = 3,scientific = T),'and tau_hat =',format(optimal_kef$tau,digits = 3,scientific = T))) +
  xlab('Value') +
  ylab('Density') +
  theme_bw() +
  theme(legend.position = "bottom")
...
#kef_res <- kef(sampled_x,grid = x_grid,lambda = 1, tau = 10^-8)
...
ggplot() +
  ...
  geom_line(data = kef_df, aes(x = grid, y = kef_pdf, color = 'KEF'), linewidth = 1) +
  scale_color_manual(name = "Type of density", values = c('True Density' = 'red', 'KDE Adaptive' = 'blue','KDE Fixed' = 'limegreen', 'KEF' = 'orange')) +
  ggtitle(paste('Kernel Density Estimate for n = ', length(sampled_x), 'lambda_hat =',
                format(optimal_kef$lambda,digits = 3,scientific = T),'and tau_hat =',format(optimal_kef$tau,digits = 3,scientific = T))) +
  xlab('Value') +
  ylab('Density') +
  theme_bw() +
...
ggplot() +
  geom_histogram(aes(x = sampled_x, y = ..density..), bins = 80, fill = 'gray', alpha = 1, color = 'black') +
  geom_line(data = true_density_df_sampled, aes(x = grid, y = true_pdf, color = 'True Density'), linewidth = 1) +
  #geom_point(data = true_density_df_sampled, aes(x = grid, y = weights_var, color = 'Weights Var'), size = 1) +
  geom_line(data = kde_adaptive_df, aes(x = grid, y = kde_adaptive_pdf, color = 'KDE Adaptive'), linewidth = 1) +
  geom_line(data = kde_fixed_df, aes(x = grid, y = kde_fixed_pdf, color = 'KDE Fixed'), linewidth = 1) +
